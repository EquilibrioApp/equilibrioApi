...
export const NotificationsScreen = ({navigation}: Props) => {
  ...
  const [appointments, setAppointments] = useState<AppointmentsDto[]>([]); //TODO cambiar dto
  const {names} = useNombrePaciente(); //Obtiene los nombres de los pacientes
  const {idPaciente, start1, start2, end1, correoPaciente, form, onChange} = //TODO nota de tipo de notificacion ejercicio, agua o comida 
    useForm({
      idPaciente: '',
      start1: '',
      start2: '',
  ...
  async function loadAppointments() {
    const idPaciente = await AsyncStorage.getItem('id');
    try {
      ...
      throw new Error(
        'Error al obtener las notificación del paciente en cuestion.',
      );
    }
  }
  ...
  async function createAppointment() {
    ...
    if (
      ...
      try {
        const resp = await inicioApi.post(`/notificacion/create`, {
          idPaciente,
          // idEspecialista,
          start,
          end,
        ...
        if (resp.status === 201) {
          setView(false);
          Alert.alert('Notificación registrada con exito.');
        } else {
          Alert.alert('Algo a salido mal al registrar la notificación.');
        }
      } catch (error) {
          ...
          <View style={styles.containerModal}>
            <View style={styles.subcontainerModal}>
              ...
              <View
                ...
                <Text style={styles.titleT}>Agendar nueva notificación</Text>
